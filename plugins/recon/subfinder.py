...
class SubfinderPlugin(ToolPlugin):
    ...
    def run(self, ctx: RunContext) -> ToolResult:
        ...
        artifact = Artifact(key="subfinder_subdomains", path=output)
        return ToolResult(tool=self.name, phase=self.phase, success=success, artifacts=[artifact], metrics={"returncode": result.returncode}, message=result.stderr.strip())