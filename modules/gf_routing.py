...
def _run_sqlmap(input_file: Path, output_file: Path) -> bool:
    ...
    for index, url in enumerate(urls, start=1):
        ...
        cmd = [sqlmap_bin, "-u", url, "--batch", f"--output-dir={sqlmap_output_dir}"]
...
def run_gf_routing(gf_dir: Path, config: dict | None = None, results_dir: Path | None = None):
    print("\n[=== PHASE 4B: GF ROUTING ===]")
    results_root = Path(results_dir) if results_dir is not None else RESULTS_DIR
    routed_dir = results_root / "vulnerabilities" / "gf_routed"
